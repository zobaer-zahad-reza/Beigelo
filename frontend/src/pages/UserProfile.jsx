...
export default function UserProfile() {
  ...
  const { user, token, getCartCount, backendUrl, currency } = useContext(ShopContext);
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      toast.info("Update functionality is a placeholder.");
      setIsEditing(false);
    } catch (err) {
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-screen text-lg">Loading your profile...</div>;
  }

  if (!user) {
    return <div className="flex items-center justify-center h-screen text-lg text-red-500">Could not load user data. Please log in again.</div>;
  }
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm p-5">
            <div className="flex justify-between items-center mb-4">
              ...
              {!isEditing ? (
                <button onClick={() => setIsEditing(true)} className="text-orange-800 hover:text-orange-600 font-medium text-sm">Edit</button>
              ) : (
                <div className="space-x-2">
                  <button onClick={handleCancel} className="text-gray-500 hover:text-gray-700 text-sm">Cancel</button>
                  <button onClick={handleSave} className="text-orange-600 hover:text-orange-800 font-medium text-sm" disabled={saving}>{saving ? "Saving..." : "Save"}</button>
                </div>
              )}
            </div>
            <div className="space-y-4">
              <div>
                ...
                {isEditing ? (
                  <input type="text" value={editedUser.name || ''} onChange={(e) => setEditedUser({ ...editedUser, name: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-orange-500"/>
                ) : (
              ...
              <div>
                ...
                {isEditing ? (
                  <input type="text" value={editedUser.phone || ""} onChange={(e) => setEditedUser({ ...editedUser, phone: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-md"/>
                ) : (
                ...
                orders.map((order) => (
                  <div key={order._id} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50">
                    <div className="flex justify-between items-start">
                      <div>
                        <p className="font-medium text-gray-800">Order ID: #{order._id.slice(-6)}</p>
