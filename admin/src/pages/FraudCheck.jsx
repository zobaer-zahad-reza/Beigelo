                    ...
                    <div className="border-t border-gray-100 bg-gray-50 p-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className={`flex items-center justify-between p-4 rounded-xl border mb-6 ${
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className={`font-bold text-lg ${result.isSafe ? 'text-emerald-700' : 'text-rose-700'}`}>
                            ...
                            {result.details && result.details.successRate !== undefined && (
                                <div className={`text-2xl font-bold ${result.isSafe ? 'text-emerald-700' : 'text-rose-700'}`}>
                        ...
                        {result.details && result.details.total > 0 ? (
                            <div className="grid grid-cols-3 gap-3 text-center">
                                <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-100">
                                    ...
                                    <p className="text-lg font-bold text-slate-800">{result.details.total}</p>
                                </div>
                                <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-100">
                                    <p className="text-xs text-gray-400 font-semibold uppercase">Delivered</p>
                                    <p className="text-lg font-bold text-emerald-600">{result.details.delivered}</p>
                                </div>
                                <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-100">
                                    <p className="text-xs text-gray-400 font-semibold uppercase">Returned</p>
                                    <p className="text-lg font-bold text-rose-600">{result.details.returned}</p>
                                </div>
                            </div>
                        ) : (
                            <div className="text-center py-4 text-gray-500 bg-white rounded-lg border border-dashed">
                                ...
                                <p className="text-xs text-blue-500 mt-1">Status: New Customer</p>
                            </div>
                        )}
