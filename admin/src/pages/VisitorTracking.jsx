...
// 🚀 FIX: Using Vite's import.meta.env to load your deployed backend URL
...
export default function VisitorTracking() {
  ...
  const handleTrack = async () => {
    ...
    try {
      ...
      setTrackMsg({
        type: "success",
        text: data.message || "Visit tracked successfully!",
      });
      fetchVisitors();
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#f4f5f7] p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Real-time server-side visitor tracking
          </p>
        </div>
      ...
      {error && (
        <div className="mb-4 bg-red-50 border border-red-200 text-red-700 rounded-xl px-4 py-3 text-sm flex items-center gap-2">
          ...
          <span>
            Failed to load visitors: <strong>{error}</strong>. Make sure your
            backend is running.
          </span>
        </div>
      )}
